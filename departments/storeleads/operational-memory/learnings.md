...
- **NEXT:** (1) Marina sends her founder-notes phrasing-principle table → bake into op-rules RULE 17 + founder-feedback format. (2) Marina sets Founder Review on the 2 Notion cards → record in `founder-feedback.md`. (3) Then: next green subcategory batch, or the **bulk-dump idea** (all green+yellow subcats + a "recently-created + revenue" cut, in a background session). Active scripts in `scripts/`: dumpers `sl_dump_full`/`sl_dump3`/`sl_html_top`, census `sl_subtree`/`sl_count`, select `sl_select`, enrich `sl_enrich2`, SH-lookup `sh_store_lookup2`. One-off discovery scripts archived in `archive/storeleads-discovery-2026-05/`.
