...
**▶ S5 (2026-06-02) — INCIDENT / УРОК. Сессия началась в пт (S4-резервуар, результаты выглядели хорошо), НО выявлена ошибка, полностью перечеркнувшая работу. Переделываем с нуля.**
- **Что случилось (кратко):** данные из скрапера были взяты НЕДОСТАТОЧНО — мы НЕ проверили, что важная инфа скрапера (полная Stage-2 карточка = баннер + top-3 товара с фото/desc/bullets) реально дошла во все пункты анализа. Ридер показывал 1 товар из 3 → 1000 магазинов (b9–b12) оценены по 1/3 данных. Двухшаговая система (скрапер stage1 → stage2-энричмент), на которую мы потратили много времени, по факту НЕ была отработана в анализе. Отчёты, построенные на этом, ненадёжны → **вердикт «0 winners» НЕДЕЙСТВИТЕЛЕН.**
- **Проверено (не гипотеза):** резервуар np_batch9-16_enriched.json — Stage 2 ПОЛНЫЙ (`tops3` до 3 товаров + `img`/`bullets`, `home_hero`; b12: 192/250 с 3 товарами, 637 картинок). Данные хорошие — ошибка в ЧТЕНИИ, не в данных. Скрапер не трогаем.
- **Урок:** перед оценкой убеждаться, что полная карточка (3 товара + картинки) дошла до анализа; отчёт должен быть тем, на что можно положиться. Хорошо, что поймали рано — учимся, обкатываем систему.
- **Дальше (после компакта Marina):** разобрать почему так вышло, начать С НАЧАЛА, тестировать пошагово — Marina видит вывод КАЖДОГО этапа (stage1/stage2/stage3). Эталон формата: `~/Desktop/StoreLeads/Nursery & Playroom (S3 2026-06-01)/np_batch1+2_stage1/2/3`. Файлы — только на Рабочий стол (StoreLeads/Nursery & Playroom), не в репо. Stage2 b9-12 уже там. Полный контекст-урок: память `feedback_storeleads_full_card_3stage.md`.
