...
- **▶ NEXT SESSION = DECOUPLE TEST ("enrich-ahead, analyze-later", Marina S4). 2 BATCHES ALREADY PRE-ENRICHED & WAITING on VPS — DO NOT re-select/re-enrich them:**
  - `np_batch7_enriched.json` (visits 167–268, 250 stores · reach 244 · needs_live 49 · A46/B22/C169 · single-DTC 48)
  - `np_batch8_enriched.json` (visits 113–167, 250 stores · reach 246 · needs_live 40 · A59/B21/C165 · single-DTC 33)
  - **Both confirmed `enriched` but NOT `processed` (0/250 in processed_domains; total still 3098)** — the decouple invariant holds.
  - **Protocol:** session starts with ZERO scraper wait → read `np_batch7_enriched.json` → read ALL + open EVERY needs_live/unreachable (same quality, RULE 23/24) → checkpoint → STOP → `sl_mark_processed` → then `np_batch8_enriched.json` same cycle. Report how it FELT (no front-load wait). THEN batches 9-10: `sl_select_all` (now correctly excludes 7+8 once marked). Selected via SKIP 0 / SKIP 250 → 0 overlap (verified).
  - If clean → validates the reservoir architecture (promotion-queue proposal #1): enrich token-free ahead, analyze on ready data.
- After b7+b8: ~2460 low-visits remain (then **2,661 MISSING-visits stores LAST** — kept, RULE 24). Full Nursery exhaust ≈ 18 more batches. Remaining = 5121 (incl. b7+b8 still unprocessed until analyzed).
