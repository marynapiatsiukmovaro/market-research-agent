...
**▶ S20 (2026-07-10) — 🛠️ ЭТАП 2 РАЗОБРАН ПО ПОЛЯМ. Карточка перестроена: два блока товаров, мёртвые поля убраны, полка названа. ▶ NEXT = ДОДЕЛАТЬ ЭТАП 2 по колонкам (описание · tier/score · валюта · product_class · фильтр нефизических · список коллекций), НЕ идти в анализ.**
*(Полная запись с цифрами → `review/s20-experience.md`. Это ОПЫТ, не аксиомы. Все скрипты — в TEST-именах, живое не тронуто.)*

- **🔴 ГЛАВНОЕ: робот читал не ту полку.** У **163 магазинов из 250** нет коллекции с ранжированием → энричер падал на `/products.json`, то есть на **складскую опись** (порядок = дата публикации). Живая проверка: `learnplay.com` через опись отдаёт **одного динозавра за $16**, а на главной — **набор за $80**, тот самый, что мы оценили в 69. Совпадение робота с витриной: **0 из 6** на всех трёх проверенных магазинах. Следствие: прогон одного батча дважды за сутки дал **30 магазинов из 250 с ДРУГИМ главным товаром**; **28 из 30 сидели на описи**. ⇒ `tier`, `score` и «0 винеров в батче» — свойство того утра, когда бегал скрапер, а не ниши.
- **✅ ПОЧИНЕНО: два блока по три (Marina).** **ПРОДАЖИ** (`/collections/all?sort_by=best-selling` — собственный рейтинг Shopify по числу заказов) + **ВИТРИНА** (`frontpage`/`featured`, иначе главная). Пересечение помечается, не схлопывается. b2: 850 товаров, 63 пересечения, охват 97.6%. **Товары в $39–170 вернулись: 93 магазина против 95 на старой лестнице** (промежуточные попытки давали 68 и 63). `thejumppad.com` — раньше №1 «ремкомплект $9», теперь видны батуты $6 600 → магазин отсеивается за 2 секунды без живого захода.
- **⚠️ ЗАКОН: охват падает от ЧИСЛА ЗАПРОСОВ, не от логики.** 5 запросов/магазин → 98.4% · 12 → 90.4% · **18 → 67.2%** · 8 → 97.6%. Терпение (таймаут 15→22 с) сделало **хуже** — оно удлиняет неудачу, а не убирает нагрузку. Лечение: один `/collections.json` вместо перебора пяти · главная качается **один раз** (качали дважды!) · таймауты 18 с × 2 · воркеров 6.
- **🔴 МЕСЯЦ МЁРТВЫХ ПОЛЕЙ — баг парсера** (проверено по git: `sl_slice_from_csv.py` создан 2026-06-08, коммит `0f41f4c` → поля мертвы во ВСЕХ CSV-срезах: H&G b1–b22, T&H b1–b4). `sl_slice_from_csv.py` делал `float("USD $150.27")` → ValueError → тихий `None`. Замер: `avg_price`/`min_price`/`max_price`/`sales` **пусты у 7639 из 7639** магазинов среза. Отсюда `SL revNone/avgNone` в каждой строке и **`maturity`, считавшийся на нуле** (после починки — `established` у 250/250, т.е. поле не несёт ни бита). Починено; срез пересобран, сверен домен-в-домен.
- **⭐ ЦЕНОВОЙ КОНВЕРТ — наше единственное преимущество над ShopHunter.** Индекс знает `min/avg/max` по ВСЕМУ каталогу. `bumpeeztoys.com`: `products.json` показывает аккумулятор $14.99 и два пульта по $9.99; индекс — `avg $150.27, max $379.99` при каталоге в 9 товаров. Машинка $149.99 в данных отсутствует. **Без флагов и порогов:** в карточке строка `каталог 9 товаров · индекс знает цены $9.99–$379.99`. Робот кладёт факт, решает агент.
- **Валюта:** правду знает только `/cart.json`. Индекс **копирует ложь магазина** (magnetichoop: индекс `USD`, корзина `HKD`) и врёт сам (27/250 говорят `USD` про UK/CA/AU-магазины). **ShopHunter конвертации не имеет вообще** — защищался щедрой полосой `$25–220` (`sh5_cut.py`: «SH price unreliable both ways») + живым заходом. **Формула: цена — ориентир, а не приговор.**
- **Как выбирает товар ShopHunter (проверено по коду):** `sh_hero_par.py` вообще не открывает магазин — читает готовый блок **«Top Products (Revenue Week)»** внутри сервиса; каталог нужен только чтобы уточнить цену. Порядок задаёт **источник, ранжированный по продажам**. Наш аналог — Shopify `sort_by=best-selling`. ⚠ Их рейтинг по ВЫРУЧКЕ, наш по ШТУКАМ → штуками лидируют расходники (у 57 из 140 магазинов товар №1 дешевле $15). Отсюда и два блока.
- **Карточка перестроена (Marina).** УДАЛЕНО: `needs_live` (решает агент, не робот) · `maturity` · `hero_confidence` · `desc_confidence`/`pos` в шапке · `invest` · `anchor` · дубли-флаги. СКРЫТО (собираем, показываем по кнопке / во втором слое): визиты · соцсети · мерчант · `new30d` · `conv` · выручка. ПОКАЗАНО: домен + питч · **полка, откуда взяты товары** · тир/score мелким · страна/дата · **каталог + конверт цен** · класс · пустышка · баннер + два блока · **внизу список магазинов, которых робот не увидел — БЕЗ тира и score** (назвать неоткрытый магазин «тиром A» — ложь; у ShopHunter они всегда шли отдельно). Размер: 1 393 → 814 симв/магазин (−42%).
- **Паритет переписан и ловит по делу.** Старый сравнивал **две захардкоженные константы**, скопированные в оба ридера. Новый выводит список полей **из факта печати** — и за сессию трижды поймал мои ошибки, включая ту, где HTML рендерил 6 товаров, а сертифицировал 3.
- **🔴 НАЙДЕНО, НЕ ПОЧИНЕНО (агенда следующей сессии, Marina: идём по колонкам, НЕ в анализ):** (1) откуда берётся **описание**; (2) формула **tier/score** — на живых данных не проверялась ни разу (magnetichoop с ложной ценой получил C|9 = дно); (3) **конвертация валюты не идеальна** — `$87.04/HKD` читается двусмысленно; (4) **`product_class` врёт**: `thejumppad` (батуты $6 600) помечен `apparel` — это прямой вход в Marina Veto; (5) **нефизические товары молча выпадают** (`tops3 = phys[:6] if phys else clean[:6]`) — противоречит founder-калибровке «в детской нише одежду не дропаем» (SH-5); (6) **список коллекций магазина собираем и выбрасываем** — показывать 5 главных + какую робот выбрал (идея Marina).
- **Правила:** убраны два устаревших хвоста, противоречивших RULE 33 (op-rules «Checkpoint shape» + workflow §S13). Больше про автономность анализа нигде не написано. **Новых правил не добавляли** (Marina: «не наслаивать правила на правила»).
- **Долги:** `sl_project_v2.py` всё ещё в `/tmp` на VPS, вне git. TEST-скрипты не узаконены. Не в Notion: Sewtalent 64 · MorisMos 55 · The Wriggler 68 · OtterSpace 66. `processed` не помечен нигде.

**▶ S19 (2026-07-09) — 🔬 ЭТАП 3, БАТЧ 2 ПРОЙДЕН С ВОССТАНОВЛЕННЫМ ОТБОРОМ. Карточка агента сжата вдвое. ▶ NEXT = разобрать Stage-2 вглубь (HTML vs текст, что оставляем, где мы сильнее ShopHunter), построить воронку явно, потом op-rules 33→~10.**
*(Полная запись опыта с цифрами → `review/s19-experience.md`. Это ОПЫТ, не аксиомы.)*

- **Причина раздутой карточки найдена.** В RULE 23 записана цель энричера v4.2 дословно: «принести достаточно контекста, чтобы уверенно пропустить магазин, **не открывая его**». Карточку откармливали, чтобы она **заменила заход**. Заход умер (RULE 29 = curl), карточка растолстела. **Одно движение, не две поломки.**
- **Замер строка-в-строку.** ShopHunter: **200 симв/магазин** (2 строки), 168 магазинов = 8.4k токенов. У нас было 1,331 симв × 250 = 88k. **Стало 699 × 250 = 43.7k (−51%).** Их ридер печатает ОДИН товар и **никогда не показывал агенту картинки** — все находки по тексту.
- **Сохранено (мы сильнее):** 3 товара вместо 1 · баннерный герой (у них нет) · питч магазина · `desc_confidence` · `needs_live`+причина · `kind`/⚠пустышка. **Убрано:** desc товаров №2/№3 (28% файла) → одно предложение · префикс URL (15%) · прокси-метрики и invest-сигналы (9%) · соцсети (4%) · мусорные флаги. Тяжёлое переехало во **слой 2** — печатается только по отобранным доменам.
- **⭐ ШАГ ОТБОРА ВЕРНУЛСЯ.** b1: 54 захода, **все из списка робота**, своих — 0. b2: **39 заходов = 17 обязательных (робот не видел магазин) + 12 МОИХ + 10 на замер потерь.** Шкала отбора записана впервые (`review/s19-experience.md` §3): один товар/своя марка · механизм виден за 3 сек · $39–170 · повторяемо у поставщика · странность в карточке · **сомневаюсь — беру.**
- **Три независимых замера про `needs_live`:** b1 — 32 «лишних» флага открыты → 0 находок; b2 — 10 случайных отброшенных вскрыты → 0 потерь; S3 — 20 вскрыты → 0 потерь. **Флаг «кривое описание» сам по себе не находит ничего; находит ВЫБОР.** Но «карточки нет вообще» (недоступен / пустой каталог / страница-заглушка) — **открываем всегда** (S16: claymore $64.95 за нечитаемым заголовком).
- **🔴 Два дефекта данных, видимых только заходом.** (a) **`/meta.json` врёт про валюту:** `magnetichoop.com` объявляет `currency: USD`, торгует в HK$ → пяльцы HK$680 (реально ≈$87) пришли как `$680 [out]`. Правило S2 «истинная валюта = `/meta.json`» **неверно.** (b) **Робот пишет страницу-заглушку в питч магазина** — 12 раз за батч («There was a problem loading this website», «connection needs to be verified», «Sign in»); помечены `needs_live` только 3. Девять выглядели нормальными магазинами.
- **Результат b2:** ACCEPT (reach 98.4%) · PARITY PASS (250·690·175). **1 winner:** Halftoys магнитный динозавр со скелетом (learnplay.com) **69** ⚠Needs Verification — чья марка, НЕ подтверждено. **2 borderline:** Sewtalent магнитные пяльцы **64** (реальная боль, но узкая аудитория + магниты = ограничения авиаперевозки) · MorisMos лягушка с органами **55** ($37, ниже пола). **9 browse.** Loss-audit 10 → 0.
- **`sl_analysis_gate.py` НЕ запускался.** По букве RULE 23 он скажет ⛔ STOP (25 флагов не открыты осознанно). **Озеленять curl-ом отказался — так родился S15.** Правило переписываем ДО того, как гейт снова станет осмысленным.
- **Открыто (решает Марина):** RULE 23 vs SH-8 · автономность (workflow стр.9 ↔ RULE 33) · op-rules 33→~10 · **b1 T&H переанализировать** (сделан без отбора → его «0 винеров» недействителен). **Не в Notion:** Halftoys 69 · Sewtalent 64 · MorisMos 55 · The Wriggler 68 · OtterSpace 66. `processed` не помечен.
- **✅ ПОЧИНЕНО В КОДЕ (S19):** `sl_enrich4.store_currency()` — спрашивает оба источника, **при расхождении побеждает `/cart.json`**; магазин получает `currency-suspect→live` + обязательный живой заход. Сквозной прогон: `magnetichoop` → HKD, **$87.04, in_range=True** (было `$680 [out]`); контрольные CHF/USD не сдвинулись. *(Конвертацию строил Store Leads в S2 — у ShopHunter её нет вообще: в `sh_enrich_final.py` слова «currency» нет ни разу.)*
- **✅ ПРАВИЛА, ИСПОЛНЕНО РЕШЕНИЕ МАРИНЫ (S19):** **RULE 29** — `sl_open_flags.py` объявлен тем, чем является: **curl-триаж «жив ли сайт», НЕ заход**; вердикт — только после настоящего живого захода; **озеленять им гейт запрещено**. **RULE 33** — «заработанная автономность» **ретайрена**: каждый батч → чекпоинт → СТОП, ждём OK. Противоречие с `workflow.md` стр.9 снято. Глубина важнее количества батчей.
- **Инструменты:** `sl_project_v2.py` (новый тонкий ридер) пока **вне репо**, живёт как черновик; `sl_project_any.py` НЕ изменён. Слой 2 (`sl_detail.py`) ещё не построен.
- **Долг:** RULE 18 — в `learnings.md` три HANDOFF (S19/S18b/S18), надо свернуть S18 в архив. И `op-rules` 33 → ~10 — отдельная сессия на свежую голову.

**▶ S18b + S18 (2026-07-09) — collapsed per RULE 18 (keep only 2 newest full = S20 + S19). Полный текст в git + `review/s18-analysis-findings.md` + `review/s18-agent-experience.md`.** 🔬 Этап 3 пройден руками на T&H b1 (0 winners · 4 borderline · 9 browse) и **найдена главная поломка: агент вообще НЕ ОТБИРАЛ магазины** — все 54 живых захода были из списка робота, своих ноль, а работа выглядела полной (гейт зелёный). 🛠️ Этапы 1–2 прощупаны руками: оба работают, сломаны были только СЛОВА о них. Исправлены три расхождения «слово ≠ дело»: два канонических ридера (агентский проектор был partial и вне git — **27 батчей агент судил по меньшему, чем видела Марина**) · один вердикт — один хозяин (баннер HTML спорил с приёмкой на одном файле) · волна 1+9, единая каденция HTML. ⭐ Родились **RULE 0b** («неудобство = дефект системы, говори сразу»; чиним КЛАСС, а не симптом; правило называет СВОЙСТВО, а не имя скрипта) и **`sl_card_parity.py`**. Урок S18b: «полнее» и «дороже» — одно движение.
