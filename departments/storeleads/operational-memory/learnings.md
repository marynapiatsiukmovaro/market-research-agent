...
**▶ S8-ANALYSIS (2026-06-05) — IN PROGRESS (read FIRST). Nursery vNone reservoir b1–b5 ANALYZED → 0 winners 65+ / 1250 stores (honest near-zero, vNone tail structurally exhausted — confirms S7). System GREEN on the series.**
- **processed = 6848** (Nursery 5344/6715 · HI 1504). **REMAINING Nursery = b6–b11 = 1371** (`np_s8_b6–b11_enriched.json`, ~6 chunks; b6–b10 = 250 ea, b11 = 121). NEXT analysis session resumes at **b6**.
- **Pipeline ran clean every batch:** sl_qa → sl_stage2_table (canonical) → read ALL 250 → sl_open_flags (needs_live+device) → live-confirm → sl_analysis_gate → checkpoint → mark_processed. **Both gates PASS ×5; ACCEPT-logic correctly handled 2 QA-STOP** (b2 reach 92%, b5 reach 90.4% = products.json vNone artifact → PROCEED+hand-open, NOT re-enrich). 343 hand-opened. **0 degradation across 5 batches.**
- **Notion writes (2026-06-05):** Yogorgeous Anti-Roll Changing Mat 53 (yogorgeous.com.au — **founder-kept 🔵 Watchlist**, "Problem exists. Just study.", WriggleBum-category) · **RockaBaby (babymarstore.com $59.90) folded as Rockit Store Link 2** (convergence, not new card). Borderlines NOT written (checkpoint-history only): numababy 58, nolele 58, babyerausa 48. keep-list +2 (yogorgeous, babymarstore).
- **⭐ PERFORMANCE OBSERVATION (Marina S8 — observe only, no change):** ~1250 stores/context before compact. **~85% of per-batch context = reading 250 full cards (RULE 6); it persists in history.** NOT scraper (0-token), NOT live-verify, NOT checkpoint. Reservoir = WALL-CLOCK win only (0 scraper wait), does NOT cut read-context. Weak tail costs same read-tokens as strong niche but 0 winners → infinite token/winner → **Cats (fresh niche) = better token-ROI**. **Hypothesis to test later (Tier-2, propose first): sub-agent reads 250 cards & returns only candidates+off-model summary → 3–4× stores/context without losing read-all guarantee.**
- *(S8-BUILD block below = how the reservoir was made. S7 = scale-validation. Older = git.)*
