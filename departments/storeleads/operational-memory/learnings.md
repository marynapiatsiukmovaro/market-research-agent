...
**▶ S18b (2026-07-09, после компакта) — 🔬 ЭТАП 3 ПРОЙДЕН РУКАМИ на T&H батч 1. GATE PASS · 0 winners · 4 borderline · 9 browse. ▶ NEXT = обсудить 4 находки ниже + ShopHunter-сравнение, ПОТОМ править op-rules.**

- **Результат b1 (T&H, визиты 9.9k–8.8k):** прочитаны все 250 карточек · **открыто вживую 54 магазина** (все 50 флагов = needs_live 38 + unreachable 12, плюс 4 на loss-audit). **0 winners.** Borderline: TrickTech Web Shooter 64 ($34.99, 100M+ показов, ниже ценового пола) · NookTales book-nook 63 ($64.99, Rolife-коммодити) · Sova One Sheet 60 ($135, дважды патент, текстиль) · Bumpeez бампер-кар 59 ($149.99, Shark Tank, громоздкий+литий). Browse 9. Loss-audit: 4 карточно-отброшенных вскрыты → 0 скрытых винеров. **Notion: пусто. processed НЕ помечен** (ждёт OK Марины).
- **Профиль ниши (наблюдение, НЕ вердикт):** на этой полосе T&H = хобби-снабжение (пряжа/ткани/бисер/керамика/краски/коллекционные фигурки/RC-запчасти/лицензионный ресейл). Магазинов «один товар, своя марка» — 6 из 250; все 4 borderline пришли оттуда.
- **⚠️ РАЗДУТИЕ КАРТОЧКИ — причина найдена, это была правка S18, не ритм и не заходы.** Марина заметила: отчёт 5,700 строк вместо привычных ~1,200. Замер на одном файле: **старый проектор 968 строк / 121k символов → S18-версия 5,696 / 528k (×4.3)**. Причина: восстанавливая полноту (banner-hero, desc 58→220, bullets, отдельная строка на URL, SL-метрики, флаги) я потерял ПЛОТНОСТЬ. **Починено (S18b): 2,724 строки / 343k, parity 28/28 полей держится.** Остаточный ×2.8 к старому — это РЕАЛЬНАЯ информация, которую старый ридер прятал. Дальнейшее сжатие до ~×1.5 возможно (desc→90, убрать URL товара) — **решение Марины: контекст vs слепота.**
  **Урок (в копилку RULE 0b):** «полнее» и «дороже» — одно и то же движение. Чиня одну крайность, проверяй вторую. Правило про full card должно называть И полноту, И бюджет.
- **🔴 ЧТО СЛОМАНО В АНАЛИЗЕ — три наблюдения из живого прогона (агенда Блока 3):**
  1. **Гейт зелен ровно тогда, когда агент делает работу, и красен, когда не делает — но озеленить его можно за минуту curl-ом.** На b1 гейт честно встал на «18/50 флагов». Прогнать `sl_open_flags` → он бы засеял все 50 строк curl-ом → ✅ PASS. Так и родился S15. Агент не стал; открыл 32 руками. **Честность гейта держится на решении агента не жульничать — это определение «системы на дисциплине».**
  2. **Правила спорят в момент работы.** RULE 23: открой КАЖДЫЙ из 50 флагов. Критерии S16 (workflow §1a): off-model-массив можно судить по карточке. Оба сразу невыполнимы. Выбран RULE 23 → **измерено: 32 живых захода, 0 находок.** Теперь решение можно принять по цифре, а не на ощупь.
  3. **Гейт считает покрытие и слеп к суждению.** Он был бы так же зелен, если бы Bumpeez выбросили строкой «off-model: игрушка». Поймал его только живой заход: робот показывал ТОЛЬКО запчасти по $9.99 (desc пустой). Ни одна проверка не спрашивает «ты вообще посмотрел, что за товар?».
- **💡 ПРЕДЛОЖЕНИЕ МАРИНЫ (S18b, к внедрению в Блоке 3) — «план открытий до открытия».** Перед живыми заходами агент ФИКСИРУЕТ число: сколько флагов робота · сколько магазинов без данных (unreachable + DROP-noPhysical) · сколько своих кандидатов по A/B/C → **ИТОГО N**. После — сверка «открыто N/N». Это счёт покрытия, а не суждения; **новых гейтов не плодим** — это одна строка в чекпоинте. Закрывает ровно ту дыру, где агент стоял на 18/50 и мог решить «хватит».
- **Ответ на вопрос «как отбирались A9/B12/C33»:** никак — их отобрал РОБОТ (`needs_live`+unreachable), тиры лишь показывают, куда эти 50 попали. Агентский выбор был только в 4 магазинах loss-audit. Магазины без данных от скрапера открыты все.
...
- **⭐ RULE 0b — «ТЕБЕ ДОЛЖНО БЫТЬ УДОБНО РАБОТАТЬ. НЕУДОБСТВО = ДЕФЕКТ, ГОВОРИ СРАЗУ» (Marina-authored S18).** Не вежливость, а
  инженерное требование: качество находок = чистота папки, в которой агент работает. Запрещённый ход — понизить противоречие до
  бумажной работы («работает, просто не задокументировано» = **никто не проверил**). Чиним КЛАСС, а не симптом. Правило называет
  **СВОЙСТВО**, а не имя скрипта — правило про инструмент ломается в ту секунду, когда инструмент стал неудобен.
- **⭐ `sl_card_parity.py` (новый; идея Марины).** Каждый батч сравнивает CERT-строки обеих поверхностей: магазины · товары ·
  баннер-герои · **28 полей контракта** (`FIELDS_RENDERED` — один список на два файла). Расхождение = ⛔ STOP («один из нас судит по
  меньшему»). **Проверено, что умеет ПАДАТЬ:** ослепил текстовый ридер на `home_hero` → STOP, т.е. воспроизвели ровно ту ошибку,
  что жила полгода. Ритуальная фраза приёмки («3 tops + images») **удалена** — она была непроверяема и ложна каждый батч (в текстовой
  проекции картинок нет вовсе); теперь в чекпоинт идут ЧИСЛА из двух проверок.
- **Дыра работала В ОБЕ СТОРОНЫ:** HTML тоже не рисовал `pust`, `kind`, `flags` и причину unreachable — а `pust`/`kind` это прямые
  входы в Marina Veto. Обе поверхности достроены до одного контракта.
- **Хронология (по mtime, не по памяти):** энричер отдаёт `home_hero` с **2026-06-01**; текстовый проектор родился **2026-06-07** —
  шестью днями позже и уже без этого поля. Он не деградировал — он таким появился. Почему выжил: правило заморозило ИНСТРУМЕНТ, а не
  свойство · файл звался `..._tmp.py` (черновики не ревьюят) · его не было в git (ни аудита, ни бэкапа, ни diff'а) · **отсутствие поля
  не имеет симптома** — урезанный отчёт выглядит целым. В `promotion-queue` (S13b) он был описан как «работает, просто не благословлено».
