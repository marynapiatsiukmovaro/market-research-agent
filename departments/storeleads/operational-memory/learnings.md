...
**▶ S21 (2026-07-10) — 🛠️ ЭТАП 2: карточка починена и доказана на живом чанке (reach 99.2%). Скрапер разбит на ДВА ЛЁГКИХ ЗАХОДА (Marina). ▶ NEXT = АНАЛИЗ b1 шаг за шагом (первый на новой карточке), потом узаконить скрипты.**
*(Полная запись → `review/s21-audit.md`. Это РАБОЧАЯ ИТЕРАЦИЯ, не финал — карточку дорабатываем через анализ, Marina S21.)*

- **⚠️ ГРОМКИЙ УКАЗАТЕЛЬ — какие скрипты действующие.** Действующая карточка = **`_TEST`-скрипты** (`sl_enrich4_TEST.py` = каркас/заход-1 + `sl_enrich_sales_TEST.py` = заход-2 + `sl_card_text_TEST.py` / `sl_card_html_TEST.py` = две поверхности). **Канонически названные живые файлы (`sl_enrich4.py`, `sl_stage2_table.py`, `sl_project_any.py`) — СТАРАЯ версия (до S20), НЕ запускать их для новой карточки.** Узаконивание в канон ОТЛОЖЕНО до конца первого анализа (Marina: сначала обкатать, потом переименовать один раз — иначе гоняем живой канон туда-сюда). Это тот же класс «слово ≠ дело», что стоил полгода — поэтому указатель здесь, наверху.
- **🔴 ГЛАВНЫЙ УРОК: reach-обвал был от ЧИСЛА ЗАПРОСОВ, не от прокси.** Мой тяжёлый один-заход (обе полки + пагинация + валюта + самопроверки ≈15 запросов/магазин) дал **reach 48%** на 250 магазинах. Marina не поверила «прокси/блип» и была права. **Контроль решил спор:** старый энричер S19 на ТЕХ ЖЕ 250 → **89.6%**. Значит виноват код, не прокси. **Правило себе: при обвале первым делом «старый код на тех же данных», а не гипотеза про внешнее.**
- **✅ ДВА ЗАХОДА (Marina S21) — построено и доказано.** **Заход 1** (`sl_enrich4_TEST.py` PASS=1, 6-й арг): каркас + 3 товара витрины, без Shopify-полки и без самопроверок, ~5–6 запросов → **reach 99.2%** (248/250, быстрее старого). **Заход 2** (`sl_enrich_sales_TEST.py`): по принятому каркасу добирает блок ПРОДАЖИ (`?sort_by=best-selling`), ~1–2 запроса → reach не тронут, у 155/250 добавлены продажи, 0 ошибок. Формула score вынесена в общий `finalize()` — оба захода считают одинаково. Файл для анализа: `th_s21_b1_P2_enriched.json`. Паритет: 250·1115 товаров·161 баннер·2 не-увидено, обе поверхности идентичны.
- **✅ 5 правок карточки (коммит `216f506`, все про «дойдёт ли товар»):** (1) фильтр нефизических УДАЛЁН — товары больше не исчезают из-за ярлыка; (2) валюта 22→44 курса, курса нет → не выдумываем доллар, «out» не ставим, → PRICE-CHECK; (3) хэндлы внутри `/cdn/` = картинки, не товары; (4) полка по СМЫСЛУ имени + дочит стр.2 collections; (5) `max<min` → «конверт не разобран».
- **🟡 ОТЛОЖЕНО до анализа (Marina):** ярлык класса товара (`apparel` на подстроке «t-**hat**») — оставлен как есть, врёт, но НИЧЕГО не прячет (фильтр убран). Формула score/нужен ли он — решим по итогам анализа. Ценовой парсер европейского формата (заглушка стоит, сам парсер — потом).
- **Долги:** узаконить TEST→канон (после анализа); доки против правил (needs_live / «28 полей» / «один блок» в 6 файлах) — волной с «op-rules 33→10». Не в Notion: Sewtalent 64 · MorisMos 55 · The Wriggler 68 · OtterSpace 66.
...
**▶ S19 (2026-07-09) — collapsed per RULE 18 (keep only 2 newest full = S21 + S20). Полный текст в git + `review/s19-experience.md`.** 🔬 Этап 3 батч 2 с восстановленным шагом отбора (39 заходов = 17 обязательных + 12 своих + 10 на замер). Карточка агента сжата вдвое (1331→699 симв). Три замера `needs_live` → флаг сам не находит, находит ВЫБОР. 🔴 Найдено: `/meta.json` врёт про валюту (magnetichoop USD↔HK$), робот пишет страницу-заглушку в питч (12/батч). ✅ Починено: `store_currency()` спрашивает оба источника, при расхождении побеждает `/cart.json`. Правила: RULE 29 (`sl_open_flags` = curl-триаж, не заход), RULE 33 (автономность ретайрена). Находки b2 (не в Notion): Halftoys 69 · Sewtalent 64 · MorisMos 55.

**▶ S18b + S18 (2026-07-09) — collapsed per RULE 18. Полный текст в git + `review/s18-analysis-findings.md` + `review/s18-agent-experience.md`.** 🔬 Этап 3 пройден руками на T&H b1 (0 winners · 4 borderline · 9 browse) и **найдена главная поломка: агент вообще НЕ ОТБИРАЛ магазины** — все 54 живых захода были из списка робота, своих ноль, а работа выглядела полной (гейт зелёный). 🛠️ Этапы 1–2 прощупаны руками: оба работают, сломаны были только СЛОВА о них. Исправлены три расхождения «слово ≠ дело»: два канонических ридера (агентский проектор был partial и вне git — **27 батчей агент судил по меньшему, чем видела Марина**) · один вердикт — один хозяин (баннер HTML спорил с приёмкой на одном файле) · волна 1+9, единая каденция HTML. ⭐ Родились **RULE 0b** («неудобство = дефект системы, говори сразу»; чиним КЛАСС, а не симптом; правило называет СВОЙСТВО, а не имя скрипта) и **`sl_card_parity.py`**. Урок S18b: «полнее» и «дороже» — одно движение.
