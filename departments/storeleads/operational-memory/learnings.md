...
**▶ S11 (2026-06-07) — 🏭 RESERVOIR-BUILD COMPLETE FOR THIS SESSION (ran PARALLEL to S12/S13b analysis — decouple PROVEN). Built Dogs 50 chunks (12,500) + Home Improvement 29 chunks (7,250) = enriched_index 19,750. NEXT: continue HI build (remaining ~18,295 of 25,545) and/or Dogs deep-vNone tail (~9,657) BY-DEMAND; tomorrow compact → paste the S11-resume prompt.**
- **Mode = 🏭 RESERVOIR-BUILD** (no analysis, no Notion). **🐶 Dogs:** 50 chunks `dogs_s1_b1..b50` = **12,500 enriched** (57% of `dogs_dedup_full` 21,907; dedup removed 78 cats-overlap, dogs∩processed=0). Paused at the thin deep-vNone bottom (reach plateaued ~87–88%, then b43 reach 83 + avgtops<2 = single-product micro-stores). Remaining ~9,657 = uniform deep-vNone → finish BY-DEMAND only (per S13b "Dogs niche weak/maybe-close"). **🔧 Home Improvement:** 29 chunks `hi_s1_b1..b29` = **7,250 enriched** (of `hi_dedup_full` **25,545**; HI dump 27,052 ALREADY existed from S2 → **no new dump, no quota hit**; dedup removed 1,507 = 1,504 already-processed HI + 3 dogs/cats). HI reach much healthier than Dogs (90–99% across all bands) but structurally heavy/trade (catalog-giant/material/part heavy, consumer-other low — RULE 11). Remaining ~18,295.
- **⭐⭐ DECOUPLED BUILD ARCHITECTURE (S11, Marina-approved) — `enriched ≠ processed` now REAL:** new **`scripts/sl_select_build.py`** (selects next 250 excluding processed ∪ `logs/storeleads/enriched_index.json`, **NO SKIP** — page defined by exclusion) + **`scripts/sl_mark_enriched.py`** (records each built chunk's domains in `enriched_index.json`). **PROVEN this session:** S12/S13b analysis ran in parallel and grew `processed` 11,219 → 17,854 **mid-build**, and the build page NEVER shifted (a marked-processed store was already in enriched_index → no-op). This RETIRES SKIP-paging for build (SKIP shifts when processed grows mid-build = coverage-gap risk). Parallel analysis of the SAME niche being built is now SAFE (guardrail: both `claude` on the Mac, never on VPS — RULE 13).
- **⭐ `scripts/sl_accept_chunk.py` (S11, Marina-approved) — per-chunk acceptance = ONE verdict line.** Wraps (does NOT replace) the existing checks: count-reconcile + **credit-guard EVERY chunk** (closed the discipline gap) + canonical `sl_qa.py` + encoded ACCEPT-logic (benign products.json/vNone STOP → ACCEPT; genuine breakage → STOP). Tested b1–b8 = reproduced manual verdicts 1-for-1. **Codified in workflow §1b "WAVE RHYTHM" + op-rules RULE 30.**
- **⭐ WAVE RHYTHM codified (workflow §1b):** chunk-1 = full SCRAPER-ACCEPTANCE → STOP/OK · chunk-2 verify · then **waves of 7** (one-line `N/7` narration per chunk via `sl_accept_chunk`) · HTML of last chunk → Desktop + consolidated report at wave end → **STOP/OK before next wave (never auto-chain waves — the "STOP between waves" rule).** Ran 9 waves clean; STOPs only on genuine breakage; count-reconcile MATCH on ALL ~79 chunks, cur_null=0 throughout, 0 worker crashes.
- **⚠️ SESSION-SCOPED acceptance flags (NOT standing policy — default stays strict 90/price-strict):** `sl_accept_chunk` gained opt-in args used ONLY this session for the thin tail — a **reach-floor arg** (used 85 then 80 for Dogs deep-vNone) and **`--price-benign`** (used for ALL HI, where trade/quote-only stores spike PRICE-CHECK). A sub-90 floor also auto-relaxes `avgtops` (single-product micro-stores) + `price`. These are Marina-OK FOR THIS SESSION only; the committed default behaviour is unchanged (floor 90, price/avgtops genuine STOPs). Revisit via promotion-queue (the S8 vNone-threshold proposal) if we ever want them standing.
- **Dedup state:** `dogs_dedup_full.json` 21,907 · `hi_dedup_full.json` 25,545 · `master_domains.json` **60,306**. `enriched_index.json` = 19,750 (Dogs 12,500 + HI 7,250).
- **🔐 PERMISSION FIX (Marina, S11):** routine VPS/read commands no longer prompt — global `~/.claude/settings.json` got broad `Bash(ssh -i ~/.ssh/market_research_vps:*)` (+scp) that tolerates `-o` flags; project `.claude/settings.json` got local read-utils + `ask` gates (git/core/Notion). Behavioural: never prefix Bash with `cd "<project>" &&` (breaks the ssh prefix-match). Memory `feedback_permission_prompts` + `feedback_stop_between_waves` updated.
- **Promotion-queue:** S11 entry added (wave-rhythm + sl_accept_chunk, Marina-approved, already applied to workflow/op-rules). The S8 vNone-threshold proposal is the home for making the session-flags standing.
