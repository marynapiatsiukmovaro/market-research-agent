...
**▶ S8 (2026-06-04) — CURRENT STATE (read FIRST). Nursery vNone reservoir COMPLETE (2621 stores) + system hardened (RULE 30 + workflow §1b). NEXT = ANALYSIS session.**
- **Reservoir built in RESERVOIR-BUILD mode (workflow §1b — NEW this session).** The entire remaining Nursery **vNone / missing-visits tail = 2621 stores** is enriched across **11 chunks** (`np_s8_b1–b11_enriched.json` in `logs/storeleads/niches/home-and-garden/nursery-playroom/`). `enriched ≠ processed` — these are **NOT** marked processed (processed stays **5598**; the ANALYSIS session marks them via sl_mark_processed after scoring). Marker on VPS: `ALL_RESERVOIR_DONE`. Per-chunk health: `np_s8_health_manifest.jsonl`.
- **Reservoir totals:** 2621 stores · **2451 reachable (93.5%)** richly-enriched cards · 170 unreachable · **627 needs_live = the hand-open set for analysis (~57/chunk)**. All 11 chunks count-reconciled (250==250; ch11=121), **0 worker crashes, cur_null=0 throughout**. QA: ch1/3/4 PASS; ch2,5–11 = numeric STOP (cand ~89–94 <95) = the **products.json-off vNone-tail artifact** (reachable cards perfect; unreachable = alive micro-stores → hand-open), ACCEPTED+note per §1b ACCEPT-logic. No proxy collapse, no real data loss.
- **▶ NEXT = ANALYSIS session over the 11 reservoir chunks** via the canonical Stage-2 pipeline (workflow §1a: sl_qa → sl_stage2_table.py → read ALL → `sl_open_flags.py` opens the ~627 needs_live + unreachable → sl_analysis_gate.py → checkpoint → Marina OK → Notion + sl_mark_processed). After this, **Nursery niche = fully exhausted (6715/6715).** Heads-up: hand-open load is heavy in the deep tail (~25/chunk in ch5/6/9) — RULE 21 (quality>tokens) applies.
- **▶ OPEN Tier-2 proposal (propose via promotion-queue, do NOT self-write):** the QA reach/cand threshold (95) is **too strict for the vNone segment** — the whole deep tail STOPs at ~90–95% by products.json-off, not by any real incompleteness. Consider a vNone-segment threshold (or treat reach≥~90 + products.json-pattern as PASS-with-note). n=11 evidence in the manifest.
- **⭐ SYSTEM HARDENED (S8, Marina-approved):** **RULE 30** (reservoir wait = `run_in_background` sentinel-wait + SSH `ServerAliveInterval=30` keepalive, NEVER foreground poll; one-chunk rhythm, no auto-chain; SKIP advances per chunk). **workflow §1b** RESERVOIR-BUILD mode (launch template · PATH-CONVENTION · 10-pt CHECK-LIST incl. run-health A/B/C · ACCEPT-logic). **`scripts/sl_master_dedup.py` + `master_domains.json`** (8219 = Nursery 6715 ∪ processed) for cross-niche dedup.
- **Findings (verified):** **"unreachable" ≈ the store has `products.json` DISABLED** (storefront ALIVE, robot can't extract → needs_live hand-open; proven S1 11/12, S2 17/18, S8 ch2 15/20) — **NOT proxy, NOT dead; a re-run does NOT recover them** (ch2 redo → 0/20, the setting is stable). Truly dead = DNS-000 / frozen-402 only. **Duration tracks unreachable-COUNT** (each burns the full 15s×2 fast-fail), not extraction volume (n=3, emerging — keep watching). Corrected workflow §1b pt4/5 (the earlier "spike = proxy" guess was refuted).
- **Cats recon (for a FUTURE parallel session — NOT yet):** Pets & Animals → **Cats = 5169** (Shopify+Active; ≥2020 a bit less). Proxy = iProyal **ISP Dedicated, 1 IP** (Test now 200/448ms). Parallel rule: enrich workers only (RULE 13), **cap TOTAL ≈ proven 8 on the single IP** (e.g. 2×4), NEVER 2×8. Cross-niche dedup via `sl_master_dedup.py dedup` before enriching the Cats dump. Launch parallel **only after** the Nursery one-chunk rhythm is proven here.
- *(S7 below = the scale-validation that made this possible. S6 = hardening. S5 = why we hardened. S4/S3 historical.)*

**▶ S7 (2026-06-03) — prior session. Scale-validation PASSED + P1/P3 tooling adopted (RULE 29) + Nursery deep-tail EXHAUSTED.**
- **b12–b16 DONE** via the canonical pipeline (sl_qa PASS → `sl_stage2_table.py` → read ALL 250 → **`sl_open_flags.py` P1 auto-opener** → `sl_analysis_gate.py` PASS). **processed = 5598** (Nursery 4094/6715; remaining ~2621 = mostly **vNone / missing-visits tail** — Marina's next target).
- **Winners → Notion (Founder Review SET by Marina S7):** 🔵 **Watchlist 3** — KefiCo Calming Koala 68 (keficompany.com) · Portable Breast Milk Cooler 65 (waiubottles.com) · BabyBond Retractable Gate 66 (babybond.ca). 🟡 **Consider 9** (S4/S6 PENDING backlog cleared now): Rockit 74 · LatchLight 76 · WonderBee Doppler 77 · KinderSense CribTent 73 · WriggleBum 68 · Roar 69 · Kanga 67 · UpPro 67 · Sleepytot 64. **S7 doppler convergence** (little-beats/bbcareshop/myfetaldoppler/baby-therapy) **folded into the WonderBee hub card** (SL2=bbcareshop + body section; WellnessBaby/FetalPlus cross-ref'd, not merged).
- **b13 + b15 + b16 = honest 0-winner** (deepest tail v5–23, apparel/keepsake/gift/food/furniture-dominated). b12 = 2 weak, b14 = 2 (gate + doppler). RELIABLE zeros (all flags opened via P1 + gates PASS). **Nursery niche tail structurally EXHAUSTED.**
- **⭐ TOOLING ADOPTED (op-rules RULE 29):** **P1** `sl_open_flags.py` (derives full flag-list + device-class, auto-seeds `opens.jsonl` with status/prices → "opened-but-not-logged" structurally impossible; fixes the b12 device gap) + **P3** transient-503 retry. **P2 functional-noun sweep DROPPED** (across b12–b15 never beat the RULE-6 full-read; on b14 would have MISSED babybond). **Founder-feedback APPROVAL FORMAT locked (RULE 17):** compact per-product blocks (`Product | Tier` / `Founder Notes:` / `Rejection Reason:` if Rejected), 4–5 words.
- **SYSTEM HEALTH REPORT = GREEN** (5/5 batches: 0 data loss, gates caught real errors twice, 0 degradation across the series). **StoreLeads v1 proven to scale without losing winners.** YELLOW only on throughput (manual read-speed) + upstream Stage-0 category-hole (in promotion-queue).
- **▶ NEXT (Marina S7):** go **DEEPER — pierce ALL remaining Nursery stores, ESPECIALLY the vNone / missing-visits stores** (unknown visit count → Marina's bet: must check them, not assume dead). After that vNone sweep → consider pivot to a new consumer niche.
- *(S6 block below = the hardening that made this scale-test possible. S5 = why we hardened; S4/S3 historical, full text in git.)*

**▶ S6 (2026-06-03) — prior session (hardening). Pipeline hardened (RULE 25–28) + b9/b10/b11 RE-RUN CORRECTLY; the broken S5 read is superseded.**
- **b9 + b10 + b11 DONE properly** via the canonical pipeline (sl_qa PASS → `sl_stage2_table.py` full card → read ALL 250 → open every flag → `sl_analysis_gate.py` PASS). **processed = 4348** (Nursery 2844/6715, remaining ~3871).
- **Winners → Notion (Founder Review PENDING Marina):** Kanga Stability Pouch **67** (thekangababy.com) · UpPro Cordless Baby Dryer **67** (uppro.co) · WriggleBum Nappy Change Harness **68** (wrigglebum.com.au — *the winner the S5 partial-read LOST; proof the hardening works*; **Jazzy Hug $38 folded as SL2 — diaper-change-restraint convergence, b11**). Bottle-warmer convergence (BabyBuddy 67 + BaeBae 66 + cross-dept Baby's Brew) **folded into BuddyBottle** card. Portable-chair convergence (Max&Koko + Miniware) **folded into Izimini** card (Miniware = SL2).
- **b11 = HONEST 0-WINNER batch** (deepest tail v29–47, almost pure apparel/gift/formula/service/decor). RELIABLE zero (all 250 full cards read + 48/48 flags opened + gate PASS) — NOT an S5-style fake zero. Borderline: breezyfeed nursing-cover-with-fan 62 (SOLD OUT), Jazzy Hug 60 (folded), NozeBot 58 (validated category but patented $134, off-model). **System verified end-to-end in-the-field (Marina's S6 test).**
- **▶ NEXT = b12** — S5 analysed it with the BROKEN read → **REDO via canonical pipeline** (workflow §1a). Reservoir intact: `np_batch12–16_enriched.json` on VPS — **DO NOT re-enrich.** b13–16 fresh after. Companion text reader = `scripts/sl_fullcard.py` (complete 3-tops, gate-certified; the canonical HTML stays `sl_stage2_table.py`).
- **#4 convergence-as-SCORE = REJECTED (Marina S6):** a 250-store batch isn't market-representative → within-batch repetition can be a false signal. Pattern-noting + Store-Link-2 folding STAY (never suppress); score/tier weighting does NOT.
- **Checkpoint standard codified** (workflow §2 "MARINA-FACING CHECKPOINT") + analysis refinements #2 (measured loss-audit) & #3 (borderline external-signal check) live.
- *(S5 block below = why we hardened. S4/S3 = historical, full text in git.)*

**▶ S5 (2026-06-02) — SESSION CLOSE-OUT + ДИАГНОЗ. Сессия ДИАГНОСТИЧЕСКАЯ: показала, что к МАСШТАБУ мы НЕ готовы. Анализ 2000 магазинов резервуара — ТОЛЬКО ПОСЛЕ утверждения структуры. План: `operational-memory/prescale-hardening-plan.md` (читать первым в S6).**

- **ЧЕСТНО, для будущего агента (без прикрас):** агент начал анализ b9–b12 и довёл его до вердикта «0 winners», НЕ заметив, что читает 1 товар из 3 без картинок. Ошибку выявила Marina, а не агент и не какая-либо проверка системы. **Если бы Marina не вмешалась и не уточнила — недостоверная работа ушла бы дальше как готовая.** Ценность сессии именно в этом: дыры вскрылись дёшево (250 магазинов), пока система не на масштабе. Это не «успешная сессия с мелким багом» — это сессия, где сам процесс не поймал крупную ошибку, и это надо помнить.

- **Что ПОДТВЕРДИЛОСЬ (хорошее):**
  - Decouple/резервуар работает: есть данные → агент стартует сразу, без ожидания скрапера (повтор S4-вывода, Marina S5 подтвердила ценность ещё раз).
  - **Скрапер/обогатитель (`sl_enrich4` v4.2) собрал ПОЛНЫЙ контракт КОРРЕКТНО** — проверено по JSON b9: все поля заполнены (3 товара + фото + desc + bullets + per-product in_range/descConf, banner, maturity, new_products_30d, cat_flag, store_type, product_class, conv, hero_confidence, storefront_pos, social). **Бага скрапера НЕТ. Данные ЦЕЛЫ — не пересобирать.**

- **Корневой диагноз (3 слоя — проверено фактом, не гипотеза):**
  - **Слой 1 — скрапер:** ✅ собрал всё (см. выше).
  - **Слой 2 — эталонная таблица S3** (`scripts/sl_stage2_table.py` = десктоп b2 «v4.2»): показывала 17 полей, но НЕ выводила 4 собранных скрапером (`store_type · product_class · new_products_30d · cat_flag`) → **неполная подача** (данные были, но не на экране). Значит даже «эталон» не равен полному контракту subagent-spec.
  - **Слой 3 — S5-регресс (корень обнуления):** агент в провальную сессию собрал КУСТАРНЫЙ `/tmp/sl_stage2_html.py` (7 колонок) + `/tmp/sl_reader.py` (печатал 1 товар из 3, без картинок) ВМЕСТО канонического генератора → 1000 магазинов (b9–b12) оценены по 1/3 данных → **вердикт «0 winners» НЕДЕЙСТВИТЕЛЕН.**

- **ДЫРЫ В СИСТЕМЕ (почему НЕ готовы к масштабу — задокументировано Marina S5):**
  1. **Нет единого канонического stage2-инструмента.** Агент может собрать ad-hoc ридер и ТИХО потерять данные. Два генератора уже разошлись (канонический vs `/tmp`).
  2. **Нет проверки выхода скрапера против эталона.** «Вчера тест прошёл, скрапер ок» = разовая ручная проверка, НЕ правило. Нужен ЭТАЛОННЫЙ контракт + АВТОПРОВЕРКА КАЖДОГО прогона на полноту (поля + 3 товара + картинки + in_range), а не только reach/price/cur — текущий `sl_qa.py` полноту карточки НЕ ловит.
  3. **stage2-карточка не финализирована до конца.** Полный набор колонок + раскладка (full vs grouped) не утверждены.
  4. **Онбординг агента.** Нет жёсткого правила «каким инструментом/документом входить» → агент может начать не с того (этой сессией так и вышло — Marina: «ты начал читать не тот документ»).

- **Артефакты сессии (на Рабочем столе `StoreLeads/Nursery & Playroom (S3 2026-06-01)`):** `np_batch9_stage1.html` (сырой дамп), `np_batch9-12_stage2.html` (кустарный, урезанный — НЕ эталон), `np_b9_MOCK_full.html` + `np_b9_MOCK_grouped.html` (макеты полного контракта, 2 раскладки — для выбора в S6). Файлы только на Рабочий стол, НЕ в репо.
- **Урок (память):** `feedback_storeleads_full_card_3stage.md` — судить по полной карточке; + теперь: скрапер был прав, ломается ПОДАЧА/ЧТЕНИЕ; нужен эталон + автопроверка.
...
- **▶ [SUPERSEDED by S6 — b9+b10 now DONE; next = b11. Kept for reservoir context.] RESERVOIR READY & QA-VERIFIED. 8 BATCHES PRE-ENRICHED (np_batch9–16_enriched.json, 2000 stores, visits 5–113 + 40 missing) on VPS at `logs/storeleads/niches/home-and-garden/nursery-playroom/`. DO NOT re-select/re-enrich.** ✅ **QA: 8/8 PASS** (`/tmp/sl_qa.py` gate — reach 96–99%, cand 96–99%, price 98–99.6%, **cur_null=0 on all 8**, avgtops 2.51–2.61); 0 overlap, 0 in processed (enriched≠processed held). Total needs_live to hand-open across the 8 ≈ **361** (~45/batch). `ALL_RESERVOIR_DONE` in `reservoir_prep.log`. `sl_qa.py` is a reusable auto-QA-gate (proposal #1 prototype).
