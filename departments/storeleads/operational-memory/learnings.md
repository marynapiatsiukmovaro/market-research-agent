...
**▶ S8 (2026-06-03) — CURRENT STATE (read FIRST). RESERVOIR-BUILD for the Nursery vNone tail + system hardening (RULE 30 + workflow §1b). IN PROGRESS — resume at chunk-4.**
- **Mode = RESERVOIR-BUILD (workflow §1b — NEW this session), NOT analysis.** Building the enriched reservoir for the Nursery **vNone / missing-visits tail** (2621 remaining) so a later session analyses ready chunks instantly. `enriched ≠ processed` — these chunks are **NOT** marked processed (processed stays **5598**; do NOT run sl_mark_processed on them during build).
- **Chunks 1–3 DONE & accepted** (750 stores enriched): ch1 reach 96.4% QA **PASS** · ch2 reach 92% (20 unreachable = **15 products.json-off ALIVE** + 5 dead → QA STOP=provisional, **ACCEPTED+note**, the 15 → needs_live hand-open at analysis) · ch3 reach 97.2% QA **PASS**. Files: `logs/storeleads/niches/home-and-garden/nursery-playroom/np_s8_b{1,2,3}_enriched.json`. **Health manifest** = `np_s8_health_manifest.jsonl` (per-chunk reach·unreachable·needs_live·duration — append each new chunk).
- **▶ NEXT = chunk-4, `SKIP=750`** (then 1000, 1250 … to 2500; ~8 chunks / ~1871 stores left of the 2621; last chunk SKIP=2500 ≈ 121). **One chunk at a time, accept via §1b SCRAPER-ACCEPTANCE CHECK-LIST, STOP for Marina each** (RULE 30). When all chunks accepted → write `ALL_RESERVOIR_DONE` marker. Launch template + PATH-CONVENTION are in workflow §1b (use verbatim — relative paths for select/enrich, cwd-path for sl_qa).
