...
- **`sl_select_build.py`** `<full_slug> <out_slug> [N=250]` — **Stage-1 (LIVE, RULE 24 + RULE 30 decoupled):** next N stores excluding processed ∪ `enriched_index`, **NO field filters**; visits = batch ORDER only (missing-visits kept, sorted last). Use THIS.
- `sl_select_all.py` `<full_slug> <out_slug> [N=250] [SKIP=0]` — *LEGACY positional selector* (excludes processed only, SKIP-paged). Superseded by `sl_select_build.py` for both build and analysis (S17); kept for reference.
- `sl_select.py` `<full_slug> <out_slug> [VLO VHI N]` — *RETIRED for niche-exhaust:* band-filtered by visits + revenue/price. Superseded by `sl_select_all.py` (RULE 24); kept only for reference.
- **`sl_enrich4.py`** `<in> <out> <sentinel> [workers=8] [limit]` — **Stage-2 live enricher (LIVE, v4.2):** open-ladder + top-3 + currency→USD + product_class (incl. `diy-home`) / store_type + homepage-hero & desc self-check + new_products_30d + product-handle + class-aware ABC. **v4.2 adds (0-token, "no store sinks first pass" — RULE 23):** home_pitch (store's own words) · home_hero (banner-featured, shown beside best-seller) · long desc + feature bullets · home_img · `needs_live` flag → agent live-opens every flagged + unreachable store. v4.1 = 8 workers + fast-fail 15s×2. (`sl_enrich3.py` = fallback; `sl_enrich2.py` = retired.)
- **`sl_mark_processed.py`** `<enriched> <subcat> <band> <date> [--monitor-min N]` — write the master store record + monitor flag (RULE 19/20).
- **`sl_qa.py`** `<enriched.json> [...]` — **Stage-2 ACCEPTANCE GATE (RULE 26, extended S6):** card-completeness audit (3 tops · per-product image/in_range/descConf · the 5 essence fields · home_pitch, not just reach/price/cur) → prints ✅ PASS / ⛔ STOP. Run BEFORE any analysis.
- **`sl_analysis_gate.py`** `<enriched.json> <opens.jsonl> <scores.jsonl>` — **ANALYSIS self-verification gate (RULE 27, S6):** verifies open-log ⊇ flags + every device-class candidate has a verdict; emits the deterministic browse-pool (RULE 28) + computes the ANALYSIS CHECKPOINT numbers. ✅ PASS / ⛔ STOP before the checkpoint.
- **The TWO canonical Stage-2 readers (RULE 25 — both full-card, both self-certifying `FULL CARD RENDERED — PASS: N/N products`; the DATA verdict belongs to `sl_qa.py`, not to them):**
  - **`sl_stage2_table.py`** `<enriched.json> <out.html> "<title>" "<funnel>"` — **FOUNDER surface:** grouped-11 HTML (Marina-locked), images, clickable → Marina's Desktop (batch 1 · every 5th · last).
  - **`sl_project_any.py`** `<enriched.json>` — **AGENT surface:** full-card text projection (250 cards fit in context, where the HTML does not). Prints banner-hero, all 3 tops, desc + bullets, `desc_confidence`/`hero_confidence`, `pust`/`kind`, unreachable reason.
  Ad-hoc/partial readers are forbidden. *(The 3 S5 `/tmp` readers — `sl_reader.py`, `sl_stage2_html.py`, `sl_stage2_v5.py` — were retired to `.RETIRED-S5`. `sl_project_tmp.py` = the VPS-only ancestor of `sl_project_any.py`, superseded S18.)*
- **Stage artifacts → Marina's Desktop (HTML, not PNG — Marina S6):** `sl_stage1_table.py` / `sl_stage2_table.py` (canonical) / `sl_stage3_pull.py` (deep-score digest) · `sl_retro.py` (retro-calibration). `sl_html2png.py` exists but PNG is no longer the default — render PNG only if Marina asks.
