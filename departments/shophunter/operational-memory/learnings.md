...
**NEXT SESSION (SH-7) options (Marina picks):** (a) **process Arts & Entertainment (823) or Toys & Games (788)** — both ALREADY dumped+verified on the VPS, run the same matured funnel (hero→cut→enrich→deep-score) + seed into "Shops" + a new niche collection; (b) **Newest-First check-up** on the now niche-organized collection (un-park the parked hypothesis — base ~100 + per-niche makes Similar/Ads/Products cleaner; scrape Products→Newest-First → seen-id dedup → cut/desc → surface NEW); (c) Explore PRODUCTS surface (different index) or geo-test US→UK/DE/AU. B&T 717 is 100% done. **NOTE: Marina has pending Notion changes to discuss/apply.**
