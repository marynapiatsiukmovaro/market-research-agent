...
- **`sl_select_all.py`** `<full_slug> <out_slug> [N=250] [SKIP=0]` — **Stage-1 (LIVE, RULE 24):** next N UNPROCESSED stores, **NO field filters** (only exclusion = already-processed); visits = batch ORDER only (missing-visits kept, sorted last). Use THIS for niche-exhaust.
...
- **`sl_stage2_table.py`** `<enriched.json> <out.html> "<title>" "<funnel>"` — **THE canonical Stage-2 reader (RULE 25, rebuilt S6):** grouped-11 layout (Marina-locked) + full v4.2 contract + a **self-certifying `STAGE-2 ACCEPTANCE` banner** in the header. Ad-hoc/partial readers are forbidden; a Stage-2 table without the banner is not canonical. *(The 3 S5 `/tmp` readers — `sl_reader.py`, `sl_stage2_html.py`, `sl_stage2_v5.py` — were retired to `.RETIRED-S5`.)*
