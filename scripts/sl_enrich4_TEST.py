...
# --- S20: the homepage sometimes isn't the homepage --------------------------------------------------
# In T&H b2, 12 stores of 250 answered the homepage fetch with a challenge/error page, and the robot filed
# that text as the store's OWN pitch: "There was a problem loading this website", "Your connection needs to
# be verified", "Sign in". Only 3 were flagged; the other nine looked like ordinary cards, and one
# (goodthreadsneedlepoint.com) sat in TIER A — the top of the read list. The card was internally consistent
# and false; nothing downstream can catch that.
# The honest distinction: these stores are NOT unreachable — /products.json answered, the catalog is real.
# Only the STOREFRONT was blocked. So we never drop them; we refuse to print a fake pitch and say so.
PLACEHOLDER = ["there was a problem loading", "connection needs to be verified", "just a moment",
               "attention required", "access denied", "enable javascript", "checking your browser",
               "please verify you are a human", "site can't be reached", "403 forbidden", "404 not found"]
PLACEHOLDER_EXACT = ["sign in", "log in", "login", "password", "coming soon", "opening soon"]

def is_placeholder(pitch):
    p = (pitch or "").strip().lower()
    if not p:
        return False
    if any(k in p for k in PLACEHOLDER):
        return True
    return p in PLACEHOLDER_EXACT          # exact match only — a real pitch may mention "login"
...
# S21: the table held 22 of the 137 currencies the index actually carries. Those 22 cover 95% of stores;
# the missing 114 cover 29,852 (5%). For a missing currency `usd()` silently used rate 1.0 — so a
# 39,900 COP pen (≈$9.85) printed as $39,900 and fell out of the $39–170 band. The error always points
# the same way: the price inflates, the store sinks to tier C, and nobody looks there.
CUR_RATE = {"USD":1.0,"GBP":1.27,"EUR":1.08,"CAD":0.73,"AUD":0.66,"NZD":0.61,"INR":0.012,"ZAR":0.055,
            ...
            "PKR":0.0036,"CLP":0.00105,"COP":0.00025,"PHP":0.0175,"MYR":0.225,"TRY":0.029,
            "EGP":0.021,"ILS":0.27,"IDR":0.000062,"PEN":0.27,"THB":0.029,"TWD":0.031,
            "SAR":0.267,"ISK":0.0073,"KWD":3.26,"ARS":0.001,"UAH":0.024,"VND":0.000039,
            "LKR":0.0034,"MAD":0.10,"NGN":0.00065,"QAR":0.275,"UYU":0.025,"GTQ":0.13}
...
def usd(price, cur):
    """(usd_price | None, currency, rate | None).

    S21, Marina: an unknown rate is never guessed. `None` says the honest thing — this price cannot be
    judged. The card then shows the native amount and NO verdict: «out» is a sentence, and we have no
    grounds to pass it.
    """
    cu = (cur or "").upper()
    if cu not in CUR_RATE:
        return None, (cur or "?"), None
    return round(price * CUR_RATE[cu], 2), cur, CUR_RATE[cu]
...
def homepage_meta(html):
    """The store's OWN pitch (og:title + H1 + og:description), the banner image, and the handle of the
    product the banner features. S20: takes the homepage HTML the caller already downloaded — we used to
    fetch the same page twice per store, which is half the reason the proxy buckled."""
    if not html:
...
HJUNK_H = ["gift-card", "gift_card", "giftcard", "personaliz", "reship", "shipping", "warranty",
           "deposit", "insurance", "protection", "sample", "subscription", "e-gift", "egift"]


def handles_from(html, n=8):
    """Product handles from a page's raw HTML, in the order the merchant put them.

    S21 — a handle is not the only thing that lives at `/products/…`. Shopify's own CDN serves product
    IMAGES from `…/cdn/shop/products/<filename>.jpg`, and the old pattern happily matched the filename,
    truncating it at the first underscore. So `bb_CharmSquaresExpanded.jpg` became the "product" `bb`,
    and `20220804_083316.jpg` became `20220804`. Both fetch nothing — but they occupied a slot in the
    3-product shelf, and a real product never got in. Measured: quiltingcat.com 7 of 65 handles were
    filenames, one of them SECOND in line; carandache.co 20 of 76.
    The fix is to look at what precedes the match: anything under `/cdn/` is a file, not a page.
    """
    out = []
    for m in re.finditer(r"/products/([a-z0-9][a-z0-9\-]{1,80})", html or ""):
        h = m.group(1)
        if "/cdn/" in (html[max(0, m.start() - 40):m.start()]):
            continue                                   # image path, not a product page
        if h in out or any(j in h for j in HJUNK_H):
            continue
        out.append(h)
        if len(out) >= n:
            break
...
def rows_from_handles(pg, dom, hs, src):
    rows = []
    for i, h in enumerate(hs):
        jp = get_json(pg, "https://%s/products/%s.json" % (dom, h), tries=2)
        if jp and jp.get("product"):
            rows.append(prod_row(jp["product"], i, src))
    return rows
...
def existing_collections(pg, dom):
    ...
    is the thing that kills reach on a single proxy IP — measured on the SAME 250 stores:
        ~5 req/store -> reach 98.4%,  8 min
        ~12 req/store -> reach 90.4%, 17 min
        ~18 req/store -> reach 67.2%, 35 min      (30 of the newly-lost stores read fine before, from a
                                                   code path that never changed -> the channel, not the logic)
    Patience does not fix congestion; it lengthens every failure. Fewer requests fixes it.

    S21: `limit=250` is Shopify's ceiling, and the list comes back alphabetical BY TITLE. tonic-studios
    has 565 collections; the answer stopped at "Hot Glue Gun". Every shelf whose title starts after H was
    invisible. We now read page 2 ONLY when page 1 came back full — one extra request for the minority of
    stores that need it, none for the rest. (The law from S20 stands: coverage falls with the NUMBER of
...
def find_shelf(colls, pattern):
    """S21: we used to test three literal names. tonic-studios calls its shelf `best-selling-products`
    and the robot walked past it — while already holding the full list of collection handles. A rule that
    enumerates spellings loses to the merchant who invents a fourth. Match the MEANING of the name."""
    return sorted(h for h in colls if h and pattern.search(h))


def sales_shelf(pg, dom, colls):
    """BLOCK A — what the store actually SELLS (Shopify's own order-count ranking).

    Mirrors ShopHunter's "Top Products (Revenue Week)": there the ranking comes from a paid revenue feed and
    the catalog only confirms the price. We have no revenue feed; Shopify ranks by UNITS. That difference is
    why we keep two blocks: units-first surfaces cheap consumables (thejumppad's #1 by units is a $9 patch
    kit, not the $600 inflatable playground it advertises).
    """
    for coll in find_shelf(colls, SALES_NAME)[:2]:      # S21: by meaning, not by a list of spellings
        j = get_json(pg, "https://%s/collections/%s/products.json?limit=6" % (dom, coll))
        if j and j.get("products"):
            return [prod_row(p, i, coll) for i, p in enumerate(j["products"])][:3], coll
    html = get_html(pg, "https://%s/collections/all?sort_by=best-selling" % dom, tries=2)
    hs = handles_from(html, 3)
    if hs:
        rows = rows_from_handles(pg, dom, hs, "shelf-best-selling")
        if rows:
            return rows, "shelf-best-selling"
    return [], ""


def front_shelf(pg, dom, colls, home_html):
    """BLOCK B — what the merchant PUSHES. `home_html` is the homepage we ALREADY downloaded for the pitch
    and the banner: fetching it a second time here is what doubled the bill for nothing."""
    for coll in ("frontpage", "featured"):
        if coll in colls:
            j = get_json(pg, "https://%s/collections/%s/products.json?limit=6" % (dom, coll))
            if j and j.get("products"):
                return [prod_row(p, i, coll) for i, p in enumerate(j["products"])][:3], coll
    hs = handles_from(home_html, 3)
    if hs:
        rows = rows_from_handles(pg, dom, hs, "homepage")
        if rows:
            return rows, "homepage"
    return [], ""


def collect_products_two_blocks(pg, dom, home_html):
    """TWO independent views of one store (Marina, S20). Neither is trusted alone.

      BLOCK A — 3 by SALES : what people buy here.
      BLOCK B — 3 by FRONT : what the merchant shows first.
      Overlap is MARKED, never deduplicated away: agreement is a signal ("they sell what they push") and
      disagreement is a stronger one (thejumppad sells repair kits and advertises a playground — both are
      the store). Nothing is dropped, no threshold, no machine verdict.

    If both blocks come back empty we fall to the catalog dump, whose order is by publish date — labelled
    "случайная выборка каталога" so the agent knows those three products prove nothing. Live proof:
    /products.json for learnplay.com hands back a single $16 dino; the homepage shows the $80 six-pack we
    actually scored 69.
    """
    colls = existing_collections(pg, dom)
    a, src_a = sales_shelf(pg, dom, colls)
    b, src_b = front_shelf(pg, dom, colls, home_html)
    if a or b:
        seen = {r.get("handle") for r in a if r.get("handle")}
        for r in b:
            r["dup_of_sales"] = bool(r.get("handle") and r["handle"] in seen)
        return a + b, (src_a or "-") + " + " + (src_b or "-"), ("high" if a else "low")
...
def collect_products(pg, dom):
    ...
    S20, measured on T&H b2 (250 stores, two runs a day apart):
      * 85 stores have a sales-ordered collection -> the pick is stable (1 of 85 changed, same 3 items).
      * 163 stores have none -> the old code fell straight to /products.json, whose order is the CATALOG's
        (roughly by publish/update date). 28 of the 30 stores that silently swapped their "hero" overnight
        sat on that rung. Its top-3 are three arbitrary items, not the store's front row.
      Probed live: glasscrafters.com — /products.json shows "spoon rest, heart plate"; the shelf sorted by
      sales shows solder, patina, copper foil (it is a stained-glass SUPPLY store). learnplay.com —
      /products.json shows a single $16 dino; the homepage shows the $80 six-pack we actually scored.

    So we now ask Shopify for the SALES ORDER before we ever read the catalog dump. This mirrors how
    ShopHunter works: `sh_hero_par.py` never picks a product off the storefront at all — it reads
    ShopHunter's own "Top Products (Revenue Week)" ranking, and the catalog is used only to confirm the
    real price. We have no revenue feed, but Shopify itself ranks by orders: `?sort_by=best-selling`.

    Returns (rows, shelf, hero_confidence). `shelf` is printed on the card so the agent knows what the
    three products are worth — a merchant's ranking, or a random slice.
    """
    # 1. the merchant's own sales-ordered / hand-curated collections (unchanged, take the FIRST items)
    ...
    # 2. Shopify's OWN sales ranking over the whole shelf — the human's "sort by popularity"
    html = get_html(pg, "https://%s/collections/all?sort_by=best-selling" % dom, tries=2)
    hs = handles_from(html, 6)
    if hs:
        rows = rows_from_handles(pg, dom, hs, "shelf-best-selling")
        if rows:
            return rows, "shelf-best-selling", "high"

    # 3. the homepage — what the merchant chose to show first
    html = get_html(pg, "https://%s/" % dom)
    hs = handles_from(html, 8)
    if hs:
        rows = rows_from_handles(pg, dom, hs, "homepage")
        if rows:
            return rows, "homepage", "low"

    # 4. last resort: the catalog dump. Order = publish date, NOT sales. Labelled as such on the card.
    j = get_json(pg, "https://%s/products.json?limit=50" % dom)
    if j and j.get("products"):
...
def work(args):
    ...
    with sync_playwright() as p:
        ...
        for d in chunk:
          try:
            time.sleep(1.2)          # S20: pace the proxy — 20 stores were lost to throttle at 0.8s
            ...
            o = {"store": (d.get("merch") or dom)[:26], "domain": dom, "country": cc,
                 ...
                 # the slice's money parser used to return None for all of them.
                 "sl_cur": d.get("cur"), "sl_min": d.get("min_price"), "sl_max": d.get("max_price"),
                 # S21: max < min is not data, it is a parse failure. The CSV stores money as a string,
                 # and 1.12% of rows use the European form `COP $111.423,08` (dot = thousands). The parser
                 # reads `111.423` and the envelope comes out inverted — carandache.co printed
                 # «$1.76 – $0.89». An impossible number must be caught, never printed. (1.23% of rows.)
                 "envelope_broken": bool(d.get("min_price") is not None and d.get("max_price") is not None
                                         and d["max_price"] < d["min_price"]),
            ...
            hm = homepage_meta(home_html)
            # S20: never write a challenge/error page into the store's pitch. Blank it, name it.
            o["homepage_blocked"] = is_placeholder(hm["pitch"])
            o["home_pitch"] = "" if o["homepage_blocked"] else hm["pitch"]
            o["home_img"] = "" if o["homepage_blocked"] else hm["img"]
            ...
            # S20: the INDEX also carries a declared currency — but Store Leads simply copies the
            # storefront's own claim, so it repeats the same lie (magnetichoop: index says USD, cart
            # says HKD). It is therefore NOT an independent source; it is a second witness to the claim.
            # Recording the disagreement makes the lie visible instead of silent.
            idx_cur = (d.get("cur") or "").upper() or None
            o["currency_idx"] = idx_cur
            o["currency_idx_mismatch"] = bool(idx_cur and store_cur and idx_cur != store_cur)
            clean = []
            for r in rows:
                if has(r["t"], JUNK): continue
                r["rate_unknown"] = False
                if r["price_raw"] <= 0:
                    r["price"] = None; r["currency"] = store_cur; r["rate"] = None; r["price_unknown"] = True
                else:
                    pu, cur, rate = usd(r["price_raw"], store_cur)
                    r["price"] = pu; r["currency"] = cur; r["rate"] = rate
                    # S21: no rate → no dollars, no verdict. The native price still reaches the card.
                    r["price_unknown"] = pu is None
                    r["rate_unknown"] = rate is None
                clean.append(r)
            ...
            # S20: no re-sorting. Block A (sales) keeps its order, block B (front) keeps its own.
            # The old `sort(pos, -invest)` would interleave the two blocks and destroy the very
            # distinction the card exists to show.
            #
            # S21 — the non-physical FILTER IS GONE (Marina). It used to read:
            #     tops3 = phys[:6] if phys else clean[:6]
            # i.e. if a store held ONE product the keyword scan called physical, every product it called
            # apparel/edible/skincare VANISHED from the card. Not down-ranked — deleted, unseen by agent
            # and founder alike. And the label itself is wrong more often than not: the word "t-HAT " makes
            # `hat` match, so a $69.95 FPV antenna is "apparel" (52 products / 17 stores in b2; 16 of the 26
            # apparel-labelled stores earned it this way). It also contradicted founder calibration
            # outright — in the baby niche apparel IS the winner (sleep sacks, swaddles, carriers, SH-5).
            # A wrong label costs a wrong sort; a deletion costs the product. We show everything we found.
            tops3 = clean[:6]
            def t_inrange(r): return (not r["price_unknown"]) and 39 <= r["price"] <= 170
            # S20: each product now says WHICH BLOCK it came from (sales shelf vs merchant's front row)
            # and whether it appears in both. Overlap is a signal, not a duplicate to be removed.
            o["tops3"] = [{"src": r.get("src"), "dup": bool(r.get("dup_of_sales")),
                           "t": r["t"][:60], "price": r["price"], "price_raw": r["price_raw"], "cur": r["currency"],
                           "rate_unknown": r.get("rate_unknown", False),
                           ...
                           "anchor": (round(100*(1-r["price"]/(r["cmp"]*r["rate"]))) if (not r["price_unknown"] and r["rate"] and r["cmp"]>r["price_raw"]>0) else 0),
                           "pust": has(r["t"]+" "+r["desc"], PUST), "desc_confidence": desc_conf(r["t"], r["desc"]),
                           "invest": r["invest"]} for r in tops3]
            # --- S20: PRICE ENVELOPE — "am I even being shown this store's real products?" -------------
            # The one signal ShopHunter structurally cannot have: it comes from the index (min/avg/max
            # price across the WHOLE catalog), not from the handful of items products.json exposes.
            # Live case: bumpeeztoys.com — products.json shows a $14.99 battery and two $9.99 remotes;
            # the index says avg $150.27, max $379.99 over 9 products. The $149.99 self-inflating bumper
            # car (the actual product, Shark Tank) never appears in the data. Only the live open found it.
            # It NEVER drops a store — it only tells the agent "the main product is not on this card".
            shown = [t["price"] for t in o["tops3"] if not t["price_unknown"] and t["price"]]
            o["hero_missing"] = False
            if shown and not o["envelope_broken"] and o.get("sl_max") and o["sl_max"] >= 39:
                if max(shown) < 39 and o["sl_max"] > 2 * max(shown):
                    o["hero_missing"] = True    # store sells real-priced goods; robot found only cheap SKUs
            if not tops3:
            ...
            if all(t["price_unknown"] for t in o["tops3"]):
                ...
                unknown_cur = any(t.get("rate_unknown") for t in o["tops3"])
                o["tier"] = "PRICE-CHECK"
                o["reason"] = ("валюта %s — курса нет, цена не пересчитана → подтвердить живьём" % store_cur
                               if unknown_cur else "products.json prices all 0 (region-gated) → confirm price live")
                res.append(o); continue
...
if __name__ == "__main__":
    ...
    for r in res:
        ...
        if r.get("currency_idx_mismatch"): flags.append("cur-idx≠cart:%s" % r.get("currency_idx"))  # S20
        if r.get("hero_missing"): flags.append("ГЛАВНОГО ТОВАРА НЕТ В КАРТОЧКЕ (индекс: max $%s)" % r.get("sl_max"))
        if r.get("homepage_blocked"): flags.append("витрина не открылась — питча нет")   # S20
