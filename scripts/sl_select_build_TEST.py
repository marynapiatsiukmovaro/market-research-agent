...
def conv(r):
    # S20: `cur` (the index's DECLARED currency) + the USD price envelope (min/avg/max) now reach the
    # enricher. Until the slice's money parser was fixed they were None for every store, so no downstream
    # step ever saw them. The envelope is the one check ShopHunter cannot make — it comes from the index,
    # not from the store's own products.json. It is how the robot can notice it is showing spare parts.
    return {"name": r["domain"], "merch": r.get("merchant"), "erf": r.get("sales"), "apf": r.get("avg_price"),
            "cur": r.get("currency"), "avg_native": r.get("avg_price_native"),
